...
for k = 1:size(f, 2)
    ...
    elseif feature_detector == "dense"
        [f1, channel1_descriptors_point] = vl_dsift(im(:,:,1), 'step', 10);
        [f2, channel2_descriptors_point] = vl_dsift(im(:,:,2), 'step', 10);
        [f3, channel3_descriptors_point] = vl_dsift(im(:,:,3), 'step', 10);   
    end
